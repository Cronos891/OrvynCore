...
class JobFamiliesAdmin(admin.ModelAdmin):
    list_display = ('job_family_name', 'job_family_group', 'job_family_status', 'tenant_id')
    list_filter = ('job_family_status',)
    search_fields = ('job_family_name', 'job_family_group')


@admin.register(JobPositions)
class JobPositionsAdmin(admin.ModelAdmin):
    list_display = ('job_code', 'job_position_name', 'job_profile', 'job_family_name')
    search_fields = ('job_code', 'job_position_name', 'job_profile')


@admin.register(BusinessUnits)
class BusinessUnitsAdmin(admin.ModelAdmin):
    list_display = ('business_unit_code', 'business_unit_name', 'tenant_id')
    search_fields = ('business_unit_code', 'business_unit_name')
...
class UnionSubgroupsAdmin(admin.ModelAdmin):
    list_display = ('union_code', 'union_name', 'tenant_id')
    search_fields = ('union_code', 'union_name')


@admin.register(WorkScheduled)
class WorkScheduledAdmin(admin.ModelAdmin):
    list_display = ('work_schedule_code', 'work_schedule_name', 'tenant_id')
    search_fields = ('work_schedule_code', 'work_schedule_name')